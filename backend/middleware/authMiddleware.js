...
// Generate JWT token - MUST include role
const generateToken = (user) => {
  return jwt.sign(
    {
      id: user._id,
      role: user.role, // ⭐ This MUST match what the middleware looks for
    },
...
// Profile, Address, and Order History functions (The Buyer CRUD)
...
export const updateProfile = async (req, res) => {
  try {
    ...
    const updated = await User.findByIdAndUpdate(req.user.id, { name, phone }, { new: true }).select("-password");
    res.json(updated);
  } catch (error) {
